...
public:
    ...
    void run() {
        ...
        while (!stop_flag) {
            ...
            //if (total_eat / N < 10) ratio = 1;
            int natempts = 0;
            double max_attempts = 100;
            //if (ratio < 0.6) max_attempts = 0;
            //if (ratio > 1.4) max_attempts = 1000000;
            while (natempts < max_attempts) {
                ...
                if (ok) {
                    ...
                    //printf("%u wt %d\n", id, int(round(1000 * eat_delay_dist.max() * ratio * ratio / 3)));
                    double ratio = N * double(eat_count) / total_eat;
                    if (total_eat / N < 5) ratio = 1;
                    usleep(round(1000 * eat_delay_dist.max() * ratio * ratio * ratio * ratio/ 4));
                    ++natempts;
                    max_attempts *= ratio * ratio * ratio;
                }
            }
