...
class LmsDataLoader(DataLoader):
    ...
    def __init__(self, objtype, table, alphabet, max_depth=4, server=None):
        ...
        self.table = table
        self.objects = {}
        # objects = self.db.query(f"""SELECT * FROM {self.table}""").dictresult()
        # for obj in objects:
    ...
    def load_all_tree(self, prefix='', depth=0):
        ...
        if len(objs) < LmsDataLoader.answLen:
            return
        for c in self.alphabet:
            self.load_all_tree(prefix + c, depth + 1)

    def addToDb(self, objects=None):
...
class LmsAuditoriumLoader(LmsDataLoader):
    ...
    def normalize_obj(self, obj):
        ...
        obj['building_id'] = self.server.get_building(building_name=descr[1])[0]['id']
                           # self.building_loader.get_building_id(descr[1])
...
class LmsLessonLoader():
    ...
    def __init__(self, auditoriumLoader, teacherLoader, server=None):
        ...
        self.table = 'lesson'
        self.lessons = {}
        # lessons = self.db.query(f"""SELECT * FROM {self.table}""").dictresult()
        # for l in lessons:
        #    self.lessons[str(l['date']) + ' ' + str(l['lesson_time_id'])] = l
...
def usage_example():
    ...
    s = student_loader.load_terms(['Орлов', 'Чупрун', 'Токмаков'])
    pp.pprint(s)
    student_loader.addToDb()
    print('\n\n\n')
    ...
    print('\n\n\n')
    # Орлов
    #l = lesson_loader.load_lessons(131075, dt.datetime.strptime('2018.12.03', "%Y.%m.%d").date(),
    #                               dt.datetime.strptime('2018.12.10', "%Y.%m.%d").date())
    #pp.pprint(l)
    print('\n\n\n')
    # Чупрун
    #l = lesson_loader.load_lessons(131116, dt.datetime.strptime('2018.12.03', "%Y.%m.%d").date(),
    #                               dt.datetime.strptime('2018.12.10', "%Y.%m.%d").date())
    #pp.pprint(l)

    building_lodaer.addToDb()
    audirotium_loader.addToDb()
    teacher_loader.addToDb()

    lesson_loader.addToDb()
